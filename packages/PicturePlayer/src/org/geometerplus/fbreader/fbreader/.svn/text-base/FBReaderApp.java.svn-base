...
public final class FBReaderApp extends ZLApplication {
	...
	public final ZLIntegerRangeOption LeftMarginOption =
		new ZLIntegerRangeOption("Options", "LeftMargin", 0, 1000, 100);//new ZLIntegerRangeOption("Options", "LeftMargin", 0, 1000, 4);
	public final ZLIntegerRangeOption RightMarginOption =
		new ZLIntegerRangeOption("Options", "RightMargin", 0, 1000, 100);
	public final ZLIntegerRangeOption TopMarginOption =
		new ZLIntegerRangeOption("Options", "TopMargin", 0, 1000, 50);//new ZLIntegerRangeOption("Options", "TopMargin", 0, 1000, 0);
	public final ZLIntegerRangeOption BottomMarginOption =
		new ZLIntegerRangeOption("Options", "BottomMargin", 0, 1000, 50);
	...
	final ZLBooleanOption SelectionEnabledOption =
		new ZLBooleanOption("Options", "IsSelectionEnabled", false);
	...
	public void initWindow() {
		...
		ZLDialogManager.Instance().wait("loadingBook", new Runnable() {
			public void run() {
				...
				//Book book = createBookForFile(ZLFile.createFileByPath("/sdcard/Books/taishan.txt"));
				if (book == null) {
...
//				BookTextView.gotoPosition(book.getStoredPosition());
				...
				if(bookmark == null)
				{
					...
					if(flag == 1)
					{
						if(	myArg0.equals(book.File.getPath()))
							bookmark = new BookmarksManager().GetBookmark();
					}
				}
	...
	public void gotoBookmark(Bookmark bookmark) {
		...
		if (modelId == null) {
			...
			Log.v("gotoBookmark", "gotoBookmark");
	...
	public void openFile(ZLFile file) {
		...
		if(flag == 1)
		{
			Book lastbook = Library.getRecentBook();
			
			if ((lastbook == null) || !lastbook.File.exists()) {
				openBook(book, null);
			}
			
			Bookmark bookmark = null;
			if(book.File.getPath().equals(lastbook.File.getPath()))
			{
				bookmark = new BookmarksManager().GetBookmark();
				if(bookmark != null)
...
//		}
	...
	public void  relevanceOpenFile(ZLFile file) {
		final Book book = createBookForFile(file);
			Book lastbook = Library.getRecentBook();
