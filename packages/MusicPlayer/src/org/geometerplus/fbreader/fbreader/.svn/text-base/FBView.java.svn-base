...
public final class FBView extends ZLTextView {
	...
	final synchronized void doShortScroll(boolean forward) {
	...
	final void doScrollPage(boolean forward){
		doScrollPage(forward, 0);
	}
	final synchronized void  doScrollPage(boolean forward, int count) {
		final ScrollingPreferences preferences = ScrollingPreferences.Instance();
		if (preferences.AnimateOption.getValue() && count == 0) {
			if (forward) {
